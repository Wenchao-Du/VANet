#PyTorch lib
""" define the generator with injected attention map
"""
import torch
...
class Generator(nn.Module):
    def __init__(self):
        super(Generator,
              self).__init__()  # first layer input channel = 2, src = 4
        self.det_conv0 = nn.Sequential(nn.Conv2d(2, 32, 3, 1, 1), nn.ReLU())
        ...
        self.det_conv_mask = nn.Sequential(nn.Conv2d(32, 1, 3, 1, 1),
                                           nn.ReLU())
        self.conv1 = nn.Sequential(nn.Conv2d(2, 64, 5, 1, 2), nn.ReLU())
        ...
        self.diconv1 = nn.Sequential(nn.Conv2d(256, 256, 3, 1, 2, dilation=2),
                                     nn.ReLU())
        self.diconv2 = nn.Sequential(nn.Conv2d(256, 256, 3, 1, 4, dilation=4),
                                     nn.ReLU())
        self.diconv3 = nn.Sequential(nn.Conv2d(256, 256, 3, 1, 8, dilation=8),
                                     nn.ReLU())
        ...
        self.deconv1 = nn.Sequential(nn.ConvTranspose2d(256, 128, 4, 2, 1),
        ...
        self.outframe1 = nn.Sequential(nn.Conv2d(256, 1, 3, 1, 1), nn.ReLU())
        self.outframe2 = nn.Sequential(nn.Conv2d(128, 1, 3, 1, 1), nn.ReLU())
        self.output = nn.Sequential(nn.Conv2d(32, 1, 3, 1, 1), nn.ReLU())
